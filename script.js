...
function renderTech(){const f=val("techFilter");const list=f==="All"?jobs:jobs.filter(j=>j.technician===f);$("techJobs").innerHTML=list.length?list.map(j=>card(j,true,false)).join(""):"No jobs assigned."}function openJob(id){const j=jobs.find(x=>x.id===id);if(!j)return;ensureTimeline(j);activeJobId=id;$("activeTitle").textContent=`${j.jobNo||""} | ${j.reg} — ${j.technician}`;$("activeDetails").textContent=`${j.type} | ${j.make||""} ${j.model||""} | Hours allowed: ${j.hours} | MOT: ${j.mot}`;
