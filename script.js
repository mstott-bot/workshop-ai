...
function render(){populateTechnicianSelects();renderManager();renderCompletedReportsInbox();renderTech();renderDash();renderTechnicianSetup();renderReportsInterface();renderDailyPlanner();loadTargetsInputs()}function renderManager(){$("managerJobs").innerHTML=jobs.length?jobs.map(j=>card(j,true,true)).join(""):"No jobs loaded yet."}
...
function renderDailyPlanner(){
  ...
  renderPlannerSuggestions(dayJobs,capacity);
}
...
if($("savePlannerSettings")) $("savePlannerSettings").addEventListener("click",()=>{
  plannerSettings.capacity=Number(val("dailyTechCapacity"))||8;
