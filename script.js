...
function render(){populateTechnicianSelects();renderManager();renderCompletedReportsInbox();renderTech();renderDash();renderTechnicianSetup();renderReportsInterface();loadTargetsInputs()}function renderManager(){$("managerJobs").innerHTML=jobs.length?jobs.map(j=>card(j,true,true)).join(""):"No jobs loaded yet."}
...
function clockOff(reason){
  ...
  j.activeClockOff={reason,start:now().toISOString()};
  addTimeline(j,`⏸ Clocked off - ${reason}`,`${j.technician} clocked off for ${reason}.`);
  save();updateClockOffStatus(j);render();
}
function clockBackOn(){
  ...
  j.interruptions=j.interruptions||[];
  j.interruptions.push({reason:j.activeClockOff.reason,start:j.activeClockOff.start,end,duration});
  addTimeline(j,`▶ Clocked back on`,`${j.technician} returned from ${j.activeClockOff.reason}. Time: ${duration.toFixed(2)} hrs.`);
  j.activeClockOff=null;
  save();openJob(j.id);render();
}
$("clockLunch").addEventListener("click",()=>clockOff("Lunch"));
$("clockIdle").addEventListener("click",()=>clockOff("Idle"));
$("clockCollecting").addEventListener("click",()=>clockOff("Collecting Car"));
$("clockBackOn").addEventListener("click",clockBackOn);
